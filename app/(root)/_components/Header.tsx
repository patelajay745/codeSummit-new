...
const Header = async () => {
  ...
  const converxUser = await convex.query(api.users.getuser, {
    userId: user?.id || "",
  });
      ...
      <div className="flex items-center lg:justify-between justify-center bg-[#0a0a0f]/80 backdrop-blur-xl p-6 mb-4 rounded-lg">
        <div className="hidden lg:flex items-center gap-8">
          <Link href="/" className="flex items-center gap-3 group relative">
            ...
            <div className="flex flex-col">
              ...
              <span className="block text-xs text-red-400/60 font-medium">
                Summit Your Code to the Top.
              </span>
            </div>
          </Link>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            <ThemeSelector />
            <LanguageSelector hasAccess={Boolean(converxUser?.isPro)} />
          </div>
          ...
          {!converxUser?.isPro && (
            ...
            >
              <Sparkles className="size-4 text-amber-400 hover:text-amber-300" />
